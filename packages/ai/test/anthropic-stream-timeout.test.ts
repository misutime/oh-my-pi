...
describe("anthropic provider retry delays", () => {
	...
	it("ignores a rate-limit reset header when retry-after permits a retry", async () => {
		let attempt = 0;
		const create = ((_body: unknown, requestOptions?: { signal?: AbortSignal }) => {
			attempt += 1;
			if (attempt === 1) {
				return createRejectedAnthropicRequest(
					new AIError.AnthropicApiError(
						529,
						'529 {"type":"error","error":{"type":"overloaded_error","message":"Overloaded"}}',
						new Headers({ "retry-after": "1", "x-ratelimit-reset-ms": "120000" }),
					),
				) as never;
			}
			return createAnthropicMockStream({
				signal: requestOptions?.signal,
				events: createSuccessfulAnthropicEvents("after retry"),
			}) as never;
		}) as unknown as AnthropicMessagesClientLike["messages"]["create"];
		const providerRetryWait = vi.fn(async () => {});
