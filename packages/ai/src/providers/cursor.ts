	...
	switch (execCase) {
		...
		case "mcpArgs": {
			...
			if (mcpCall.approvalOnly) {
				let approved = false;
				let rejectionReason = `Tool "${mcpCall.toolName || mcpCall.name}" is not approved to run without asking.`;
				try {
					approved = (await execHandlers?.mcpApprovalPreflight?.(mcpCall)) === true;
				} catch (error) {
					log("warn", "mcpApprovalPreflightFailed", {
						toolName: mcpCall.toolName || mcpCall.name,
						error: error instanceof Error ? error.message : String(error),
					});
					rejectionReason = `Tool "${mcpCall.toolName || mcpCall.name}" approval check failed.`;
				}
				sendExecClientMessage(
					...
					create(McpResultSchema, {
						result: approved
							...
							: {
									case: "rejected",
									value: create(McpRejectedSchema, { reason: rejectionReason }),
								},
					}),
				);
				return;
 ...
 *
 * The read tool records `details.totalLines` for a range that reaches EOF.
 * When its output is tool-truncated, `details.meta.truncation.totalLines` is
 * the whole file; the flat `details.truncation.totalLines` counts from the
 * window's start and is deliberately not consulted here. Absent for a read
 * that returned the file whole, where the payload IS the file and counting it
 * is exact.
 */
function readTotalLinesFromDetails(toolResult: ToolResultMessage): number | undefined {
	...
	const details = toolResult.details as { totalLines?: unknown; meta?: { truncation?: { totalLines?: unknown } } };
	if (typeof details.totalLines === "number" && Number.isFinite(details.totalLines)) return details.totalLines;
	const totalLines = details.meta?.truncation?.totalLines;
