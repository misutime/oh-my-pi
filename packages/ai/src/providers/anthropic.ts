...
function retryDelayFromAnthropicError(error: unknown): number | undefined {
	const headers = getHeadersFromError(error);
	if (!headers) return undefined;
	if (headers instanceof Headers) return retryDelayFromHeaders(headers);
	return retryDelayFromHeaders({
		...
		try {
			...
			while (true) {
				...
				try {
					...
					const headerDelayMs = retryDelayFromAnthropicError(streamFailure);
