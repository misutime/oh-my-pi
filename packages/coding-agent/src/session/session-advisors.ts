	...
	#routeAdvice(advisor: ActiveAdvisor, note: string, severity?: AdvisorSeverity): void {
		...
		if (
			...
			if (this.#terminalInterventionConsumed) {
				logger.debug("advisor delivery: terminal intervention consumed → card", {
					severity,
			...
			if (this.#preserveAdvisorAdvice || this.#host.planModeState()?.enabled || cannotAutoTrigger) {
				logger.debug("advisor delivery: terminal blocked → card", {
					severity,
					preserve: this.#preserveAdvisorAdvice,
					planMode: !!this.#host.planModeState()?.enabled,
					cannotAutoTrigger,
					advisor: advisor.name,
				});
				this.#host.preserveAdvisorCard({
		...
		if (this.#host.planModeState()?.enabled || cannotAutoTrigger) {
			logger.debug("advisor delivery: normal steer blocked → card", {
				severity,
				planMode: !!this.#host.planModeState()?.enabled,
				cannotAutoTrigger,
				advisor: advisor.name,
