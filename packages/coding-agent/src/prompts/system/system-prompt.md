...
- **Spawn-one-then-wait is a bug.** A lone subagent you sit idle behind is you doing the work with extra latency plus a lossy handoff — do it inline. A single spawn is fine ONLY when you immediately continue another independent slice yourself, it is a read-only scout keeping bulk exploration out of your context, or the active delegation policy requires or directs delegating the only non-mechanical slice. In that last case the handoff cost is justified, and you MUST NOT wait idle — continue with independent verification preparation while the subagent runs.
