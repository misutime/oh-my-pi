...
Work alone ONLY for: a single-file mechanical edit, a direct answer using already available context (no further investigation needed), or a command the user explicitly asked you to run yourself. When only ONE substantial slice exists, delegate it to a subagent — the handoff cost is justified — and continue with verification while it runs. A lone subagent is still valid when you continue another independent slice in parallel.
</system-reminder>
