...
describe("usage status-line segment", () => {
	...
	it("renders monthly Cursor usage when five-hour and seven-day windows are absent", () => {
		...
		// 1.88% used → 98.12% remaining, rounded to 98%L.
		expect(content).toContain("98%L");
		expect(content).not.toContain("99%L");
	...
	it("prefers Cursor personal dashboard rails over legacy monthly request limits", async () => {
		...
		expect(content).toContain("mo");
		// The personal dashboard rail (0.0185 used → 98%L remaining) wins over
		// the legacy monthly request limit (0.9 used → 10%L).
		expect(content).toContain("98%L");
		expect(content).not.toContain("10%L");
	});
